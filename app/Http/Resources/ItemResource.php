...
class ItemResource extends JsonResource
{
    ...
    {
        $taxRate = $this->relationLoaded('taxes') && $this->taxes->isNotEmpty() ? (float) $this->taxes->sum('rate') : 0.0;
        ...
        if ($this->product_type === 'simple_product') {
            ...
            $itemBase = $this->variants->map(fn($v) => $v->sale_price ?? $v->regular_price ?? $v->price)->filter()->min();
        }
